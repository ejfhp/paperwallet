ninja.wallets.paperwallet = {
		...
		var walletHtml =
							"<div class='public'>" +
								...
								"<div class='pubaddress'>" +
									...
									"<span class='output' id='bitcoinaddress" + i + "'></span>" +
								"</div>" +
							"</div>" +
							"<div class='private'>" +
								...
								"<div class='privwif'>" +
									...
									"<span class='output' id='bitcoinprivwif" + i + "'></span>" +
								"</div>" +
							"</div>";
		return walletHtml;
	},
	...
	showWallet: function (idPostFix, bitcoinAddress, privateKey) {
		...
		document.getElementById("bitcoinprivwif" + idPostFix).innerHTML = privateKey;
		var keyValuePair = {};
	...
	templateArtisticHtml: function (i) {
		...
		//"<iframe src='bitcoin-wallet-01.svg' id='papersvg" + i + "' class='papersvg' ></iframe>" +
								...
								"<div id='qrcode_public" + i + "' class='qrcode_public'></div>" +
								"<div id='qrcode_private" + i + "' class='qrcode_private'></div>" +
								"<div class='bitcoinaddress' id='bitcoinaddress" + i + "'></div>" +
	...
	showArtisticWallet: function (idPostFix, bitcoinAddress, privateKey) {
		...
		if (ninja.wallets.paperwallet.encrypt) {
			...
			document.getElementById("bitcoinencryptedkey" + idPostFix).innerHTML = privateKey.slice(0, half) + '<br />' + privateKey.slice(half);
		}
		else {
			document.getElementById("bitcoinprivwif" + idPostFix).innerHTML = privateKey;
		}
